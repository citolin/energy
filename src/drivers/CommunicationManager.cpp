...
void CommunicationManager::onSerialCallback(String data)
{
    ...
    values = this->callback( EVENT_READ_MEASURES , params);    
    Serial.println("Reading measured values:");
    printUnorderedMap(values);

    Serial.println("Reading broadcast frequency:");
    values = this->callback( EVENT_READ_BROADCAST_FREQ, params );
    printUnorderedMap(values);
}
